...
gamma_pars;
c=[1 1/2 1/4 1/16 1/32 1/64];

for rr=1:size(tau,1)
    for qq=1:length(c)
        for j=1:ntrials

            input_e=poissrnd(rmax*(c(qq)^2/(c(qq)^2+c_50^2))*sc*((1+cos(theta)^2)/2),16000,1);
            input_i=poissrnd(rmax*(c(qq)^2/(c(qq)^2+c_50^2))*sc*((1+cos(theta)^2)/2),16000,1)*0.8;
