...
for rr=1:size(tau,1)
    for qq=1:length(r)
        for j=1:ntrials
            ...
            input_e=poissrnd(rmax*(c^2/(c^2+c_50^2))*sc*((1+cos(theta)^2)/2),16000,1);
            input_i=poissrnd(rmax*(c^2/(c^2+c_50^2))*sc*((1+cos(theta)^2)/2),16000,1)*0.8;
            ...
            for t=2:length(tspan),
                ...
                gdot = (-G(t-1,j) + weg*max([E(t-1,j) 0])*sc*r(qq)^2)/tau(rr,3);
